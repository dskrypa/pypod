"""
This module is used to create self-signed certs to validate SSL traffic to/from a local iDevice.  It was adapted from
the original version that used M2Crypto, which has dependencies that are difficult to build/install on some systems.
This version uses openssl instead, and removes unnecessary copypasta that the original had from a tutorial on creating
